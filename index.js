...
const client = new WAConnection()
   ...
   console.log(color('☆','green'),color('♡','red'),color('🍁','white'),color('JANGAN LUPA SUBS CHANEL','white'),color('YOU','red'),color('TUBE','white'),color('AMINAH BOTZ','yellow'))
})
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '*「 ❗ 」Fitur Ini Hanya Untuk Penggunaan Mods Aminah',
...
const levelRole = getLevelingLevel(sender)
        ...
        if (levelRole <= 2) {
            ...
            role = 'Awakend Mythical Glory 特尔邦贡'
...
if (bad.includes(messagesC)){
	...
	setTimeout( () => {
	    client.updatePresence(from, Presence.composing)
	    reply("「 MENGENANG 」KITA DOAKAN SEMOGA DIA MENDAPATKAN HIDAYAH AGAR  TIDAK MNEGUCAPKAN BIACARA KASAR.")
	}, 0)
    }
	...
	// ANTI LINK GRUP
		        ...
		        setTimeout( () => {
			        client.groupRemove(from, [kic]).catch((e)=>{reply(`*AYUKINO  HARUS JADI ADMIN❗*`)})
		        }, 3000)
		        setTimeout( () => {
			        client.updatePresence(from, Presence.composing)
			        reply("Hedsot....")
...
┌──「 *USER* 」
│
├ ╰❥*${prefix}reqwest [opsional]*
├ ╰❥*${prefix}laporbug [opsional]*
├ ╰❥*${prefix}infoalamat [opsional]*
...
├ ╰❥*${prefix}lirik [opsional]*
├ ╰❥*${prefix}modroid [opsional]*
...
├ ╰❥*${prefix}tahta [text]*
├ ╰❥*${prefix}tahta2 [text]*
├ ╰❥*${prefix}pronlogo [text]*
├ ╰❥*${prefix}snow [text]*
├ ╰❥*${prefix}marvelogo [text]*
├ ╰❥*${prefix}mlogo [text]*
├ ╰❥*${prefix}text3d [text]*
...
├ ╰❥*${prefix}embuntext [text] *
├ ╰❥*${prefix}textsky [text]*
├ ╰❥*${prefix}textblue [text]*
├ ╰❥*${prefix}textdark [text*
├ ╰❥*${prefix}apiteks [text*
├ ╰❥*${prefix}quoteit [tx|tx|random]*
├ ╰❥*${prefix}marvelogo [text]*
...
├ ╰❥*${prefix}quran*
├ ╰❥*${prefix}jsholat [daerah]*
├ ╰❥*${prefix}murrotal [surah]*
├ ╰❥*${prefix}sholawat [judul]*
├ ╰❥*${prefix}videodakwah [hukum memakai cadar]*
└ ╰❥*${prefix}listsurah*
...
85. Oʀᴇ ɴᴏ Iᴍᴏᴜᴛᴏ ɢᴀ Kᴏɴɴᴀɴɪ Kᴀᴡᴀɪɪ Wᴀᴋᴇ ɢᴀ Nᴀɪ`, text, tescuk, cr)
		...
		if (isBanned) return reply(ind.baned())
                ...
                await reply( `Partner Ketemu😉: 🗿\n*${prefix}next* — New partner AMINAH `)
            break
		case 'report':
                    ...
                    reply('*🍁DEAR USER🍁*\n*━━━━━━━━━━━━━━━━━━━━*\nMasalah telah di laporkan ke owner BOT AMINAH , laporan palsu/main² tidak akan ditanggapi.\n*━━━━━━━━━━━━━━━━━━━━*')
                    break
            case 'next':
		if (isBanned) return reply(ind.baned())
                if (!isRegistered) return reply( ind.noregis())
...
if (!isPublic) return reply(mess.only.publikG)
				...
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BC GROUP AMINAH  」*\n\nDari Grup : ${groupName}\nPengirim : wa.me/${(sender.split('@')[0])}\nPesan : ${body.slice(6)}\nNote:Jangan Lupa Donasi:D`)
						}
		    ...
		    reply('Terima Kasih Sudah Daftar')
            	...
            	case 'mining':
            	  if (!isPublic) return reply(mess.only.publikG)
                      ...
                      reply(`karena kamu owner bot AMINAH  ini ${one}Xp untuk kamu😉`)
                      }else{
...
case 'public':
          ...
          case 'ping':
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ *BROADCAST AMINAH * ❯\n\n${body.slice(4)}`})
						}
            ...
            case 'creator':
                 ...
                 client.sendMessage(from, tod, image, { quoted: mek, caption: '*Halo Sobat! Saya Ownernya Kalau Mau Tanya Tanya Chat Aja Ya Sob!, Salam kenal 🤗*'})
                 break
					break    
           case 'setname':
                if (!isGroup) return reply(ind.groupo())
